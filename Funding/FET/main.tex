...
\title{$\mathcal{ROBHOOT}$ \\ Automated Discovery-Knowledge Graphs in
  Federated Cooperation Networks \\ v.2.0}} % The article title
...
\begin{comment}
 ...
 %Victor comments
  {\bf No veo que la fig. 1 ayude a responder a la excelencia de esta
    seccion; \\ No deber�amos ser capaces de destacar la excelencia
    sin la tabla 1, es decir, sin tecnicismos???}

  We are in the midst of the fourth industrial revolution, a
  transformation revolving around data-driven intelligent
  machines. More than half (i.e., 3.9 billion) of the global
  population is now online and using the Internet, which represents a
  more inclusive global information society {\bf Ref needed}. People
  are applying technology for good in powerful ways, from adopting
  decentralized technologies for humanitarian efforts to improving
  agricultural practices and reducing waste in the global food supply
  chain [2]. Big Data analytics is advancing at a pace dictated by the
  availability of data and at the moment a myriad number of approaches
  are being developed to extract information from data.  Most of these
  approaches focus on extracting patterns from one or a few
  sources. Recently, the most common approach extracts pattern from
  data although recently, Artificial intelligence based on processes
  are being proposed.  However, despite the rapid evolution of the
  digital ecosystem most BiG Data and AI approaches remain local, in
  terms of data sources and algorithmic approaches. In the scientific
  arena, research is performed locally [1] {\bf Este es m�s reciente
    Maisonobe, M., Eckert, D., Grossetti, M., J�gou, L., & Milard,
    B. (2016). The world network of scientific collaborations between
    cities: domestic or international dynamics?. Journal of
    Informetrics, 10(4), 1025-1036.}. Yet, the current technological
  paradigm assisting humans for scientific inquiry is currently based
  on competitive schemes instead of intelligent global collaborative
  protocols in federated networks [3]. {\bf desarrollar la frase que
    sigue} Besides, technologies for scientific inquiry are highly
  fragmented, partly solve reproducibility, are mostly developed in
  closed-source software, and contain many biases [4, 5, 6, 7, 8, 9,
  10]. Thus, despite the importance of global access to discovery to
  close knowledge gaps for rapid information access in emergency and
  ...
  Despite the technological advances in the field {\bf Aqu� podemos
    ilustrar las ebilidades poniendo como ejemplo los putos d�biles de
    c�mo se ha tratado el covid.}, the transformation of a digital
  society into a knowledge-based society requires solving several
  gaps. First, data integration: to leverage the abundance of data
  worldwide technology should be able to integrate data from different
  sources and in different formats and make it available for
  analysis. Second, the analysis of the data should go beyond the
  identification of patterns and consider approaches delivering
  process, and the comparison of several processes in an automated
  fashion, that is, independent from the end-user. Third, to leverage
  the computing capacity the analysis should be performed ina federate
  way, such that platform can learn from each other. Finally, the
  whole process should be made available for reproducibility and
  transparency such that benefit from the collective. This lack of
  reproducible technologies is particularly relevant for targeting
  rapid information access in global emergency and sustainability
  challenges.

  {\bf esto no lo veo} How can automated and explainable (Table 1)
  data- and causal- knowledge graphs be integrated into discovery
  networks to generate predictive scenarios for global emerging
  challenges? How can discovery-driven intelligent machines help to
  reach global sustainability goals? The project integrates data- and
  causal-knowledge graphs, the discovery-knowledge graph, into
  intelligent networks to provide critical and accessible information
  to help humanity to make informed decisions in emergency and
  sustainability challenges (Figure 1 and Table 1).

  The goal is the foundation of a new science-based discovery
  technology. This technology integrates data, and develops
  causal-knowledge graphs and delivers reproducible reports which will
  facilitate governance in rapidly changing landscapes. The main
  objectives will require four intermediate objectives: (Figures 1, 2,
  and 4) {\bf se cita antes la Fig 4 que la 3}: - RH1.0: to deploy
  question- and data-knowledge graphs for understanding bias and
  diversification of information sources when obtaining global
  data-architecture maps (section 3.1).  - RH2.0: to integrates
  automated and explainable biology-inspired neural-networks to
  decipher causal- knowledge graphs from data-knowledge graphs
  (section 3.2).  - RH3.0 to implement decentralization and sharing
  data- and causal-knowledge graphs protocols for discovery in
  federated networks (section 3.3), - RH.4.0 to integrates automated
  discovery-knowledge graphs in federated cooperation networks.

  Concept Meaning Question-knowledge graph Technology-driven
  information extraction from a corpus or similar to detect question
  gaps in multidisciplinary research Data-knowledge graph
  Technology-driven information extraction from diverse data-sources
  to infer global data-architecture Causal-knowledge graph
  Technology-driven information extraction to provide
  explain-able/interpretable scenarios on global and complex
  sustainability challenges Discovery-knowledge multigraph Novel
  interactions emerging from the integration of data- and
  causal-knowledge graphs to provide multidisciplinary responses to
  global sustainability challenges Automation Algorithms targeting
  minimal human-driven interference Knowledge-inspired society
  Open-access discovery to make informed decisions in global
  sustainability challenges Neutral-knowledge generation Open
  reproducible reports making transparent the many sources of bias in
  the discovery process T able 1: Glossary of terms.
\end{comment}
...
\begin{comment}
  We are in a moment for robust scientific collaboration for a more
rapid and less bias reaction to global challenges.

Why EU should invest in this project? Which is the case study -- Why should
individuals invest in this project?

  Robust scientific collaboration for more rapid reaction to global
  challenges
  
  Scientific responses from a complementary set of angles: Vaccines,
  immune system, epidemiology, ecology and biodiversity

  Origin pandemic: eating or from farms but the empirical evidence
  suggest biodiversity loss ---- increases the probability of zoonosis
 
  This transition better - from the existing gaps to the current
  example of the pandemic response - This teaches us science and
  technology respond partially and in fragmented parts -- for example
  producing many independent webpages offering data, more
  sophisticated Knowledge Graphs (ref), and many epidemiological
  models with different assumptions (julia model, etc) and animations
  procedures. Partial and fragmented responses offer an opportunity to
  build more integrative and innovative technologies, for example,
  connecting Knowledge graphs to more general modeling scenarios to
  \end{comment}
