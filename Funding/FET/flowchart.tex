...
startstop/.style = {base, rounded corners, fill=yellow!90,minimum height=20mm,text width=65mm},
 ...
 decision/.style = {diamond, aspect=1.5,
    ...
    \node (LABE2)    [processE2, above right=of start]  {};
                        \node (start)  [rectangle,startstop]                 {\Large {{\bf I'M $\mathcal{ROBHOOT}$\\\vspace{0.15 in} OPEN RESEARCH NETWORK}}};
                        \node (LAB1)    [processL, below=of start]  {{\Large {\bf WHY?}}};
                        
%-----------------------------------------------------------%
\begin{scope}[io/.default = 10em/13mm]
  \node (in1) [io, below left=of LAB1] {{\bf GLOBAL\\ RESEARCH\\
      KNOWLEDGE\\ GAP:\vspace{0.25 in} \\CENTRALIZATION\\ BIAS\\
      ERROR-PRONE\\ NONREPRODUCIBILITY\\ AND\\ LACK OF INCENTIVES}};
%\node (in2) [io, below =of in1]        {{\bf CENTRALIZATION, BIAS, ERROR, NONREPRODUCIBILITY AND LACK INCENTIVES}};
  \node (in3) [io, below right=of LAB1] {{\bf GLOBAL
      \\ SCIENCE\\ REPORTS: \vspace{0.37 in} \\OPEN-ACCESS\\
      REPRODUCIBLE\\ LOCAL\\REGIONAL\\GLOBAL SCALES (NGOs,GOs)}};
    \end{scope}
%-----------------------------------------------------------%
...
\draw [arrow,line width=1mm]   (start)   edge (LAB1)
                ...
                (comparison)  to (stops);
%-----------------------------------------------------------%
                \draw [arrow,red,line width=2.5mm] (LAB1) -| (in1) node[midway,above]{{\Large {\bf REDUCE}}};              % around trapecium
                \draw [arrow,OliveGreen,line width=2.5mm] (LAB1) -| (in3) node[midway,above]{{\Large {\bf INCREASE}}};
